...
class CryptoIntegration:
    ...
    async def encrypt(self, data: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(
                f"Encrypting {len(data)} bytes using component '{self._crypto_component_name}'"
            )
            ...
            encrypt_response = await client.encrypt(
                ...
                key_name="cache-encryption-key",  # Default key name
                algorithm="AES-GCM-256"  # Default algorithm
            )
            ...
            logger.error(
                f"Dapr client not available for cryptography: {e}. "
                "Falling back to plaintext storage."
            )
            self._crypto_available = False
            raise DaprCryptoError(f"Dapr client not available: {e}")
            ...
            raise DaprCryptoError(f"Encryption failed: {e}")
    ...
    async def decrypt(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(
                f"Decrypting {len(encrypted_data)} bytes using component '{self._crypto_component_name}'"
            )
            ...
            logger.error(
                f"Failed to decrypt data using component '{self._crypto_component_name}': {e}. "
                "Treating as cache miss."
            )
            raise DaprCryptoError(f"Decryption failed: {e}")
    ...
    async def _get_dapr_client(self) -> Any:
        ...
        if self._dapr_client is None:
            try:
                ...
                raise ImportError(f"Dapr client not available: {e}")
