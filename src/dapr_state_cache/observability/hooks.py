    ...
    def on_cache_hit(self, key: str, latency: float) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                logger.warning(
                    "Hook error in on_cache_hit for key '%s': %s",
                    key, e
                )
    ...
    def on_cache_miss(self, key: str, latency: float) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                logger.warning(
                    "Hook error in on_cache_miss for key '%s': %s",
                    key, e
                )
    ...
    def on_cache_write(self, key: str, size: int) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                logger.warning(
                    "Hook error in on_cache_write for key '%s': %s",
                    key, e
                )
    ...
    def on_cache_error(self, key: str, error: Exception) -> None:
        ...
        for hook in self._hooks:
            try:
                ...
                logger.warning(
                    "Hook error in on_cache_error for key '%s': %s",
                    key, e
                )
