...
class CacheStats:
    ...
    def average_hit_latency_ms(self) -> float:
        ...
        return (
            sum(self.hit_latencies) / len(self.hit_latencies) * 1000
    ...
    def average_miss_latency_ms(self) -> float:
        ...
        return (
            sum(self.miss_latencies) / len(self.miss_latencies) * 1000
    ...
    def average_write_size_bytes(self) -> float:
        ...
        return (
            sum(self.write_sizes) / len(self.write_sizes)
            if self.write_sizes else 0.0
        )
...
class CacheMetrics:
    ...
    def get_key_stats(self, key: str) -> Optional[CacheStats]:
        ...
        with self._lock:
            ...
            return [
                TopKeysResult(
                    key=key,
                    count=stats.hits,
                    percentage=stats.hits / total_hits * 100 if total_hits > 0 else 0.0
                )
...
class MetricsCollectorHooks:
    ...
    def __init__(self, metrics: Optional[CacheMetrics] = None) -> None:
